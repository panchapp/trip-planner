...
- Use `@tailwind` directives only in the project's global Tailwind entry (e.g. `src/tailwind.css`).
...
- Do not create or reference `*.scss`, `*.css` (except the global `tailwind.css`), or component `styleUrl`s.
